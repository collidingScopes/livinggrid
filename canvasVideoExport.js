...
function saveImage(){
  ...
  const date = new Date();
  const filename = `livingGrid_${date.toLocaleDateString()}_${date.toLocaleTimeString()}.png`;
...
a.href = url;
const date = new Date();
const filename = `livingGrid_${date.toLocaleDateString()}_${date.toLocaleTimeString()}.mp4`;
a.download = filename;
